    ...
    for (id key in dict) {
        ...
        if ([propertyType isEqualToString:@"NSString"] \
            ...
            // Else, set value for key
                ...
                if ([propertyType isEqualToString:@"l"] && [value isKindOfClass:[NSString class]] && [value isKindOfClass:[NSMutableString class]]) {
                    value = [NSNumber numberWithLong:(long)[((NSString *)value) longLongValue]];
                }
...
+ (id)objectWithClass:(Class)cls fromDictionary:(NSDictionary *)dict exclude:(NSArray *)excludeArray {
    id obj = [[cls alloc] init];
    
    [RMMapper populateObject:obj fromDictionary:dict exclude:excludeArray];
    
    if ([obj respondsToSelector:@selector(reloadData)]) {
        [obj performSelector:@selector(reloadData) withObject:nil];
    }
...
+(NSArray *)arrayOfClass:(Class)cls fromArrayOfDictionary:(NSArray *)array exclude:(NSArray *)excludeArray {
    NSMutableArray *mutableArray = [RMMapper mutableArrayOfClass:cls fromArrayOfDictionary:array exclude:excludeArray];
...
+(NSMutableArray *)mutableArrayOfClass:(Class)cls fromArrayOfDictionary:(NSArray *)array exclude:(NSArray *)excludeArray {
    ...
    for (id item in array) {
        ...
        // Convert item dictionary to object with predefined class
        id obj = [RMMapper objectWithClass:cls fromDictionary:item exclude:excludeArray];
        [mutableArray addObject:obj];
    }
