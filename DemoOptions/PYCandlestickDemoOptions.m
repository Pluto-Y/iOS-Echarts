...
@implementation PYCandlestickDemoOptions

+ (PYOption *)candlestick1Option {
    return [PYOption initPYOptionWithBlock:^(PYOption *option) {
        option.titleEqual([PYTitle initPYTitleWithBlock:^(PYTitle *title) {
            title.textEqual(@"2013年上半年上证指数");
        }])
        ...
        .tooltipEqual([PYTooltip initPYTooltipWithBlock:^(PYTooltip *tooltip) {
            ...
            .formatterEqual(@"(function (params) {var res = params[0].seriesName + \' \' + params[0].name;res += \'<br/>  开盘 : \' + params[0].value[0] + \'  最高 : \' + params[0].value[3];res += \'<br/>  收盘 : \' + params[0].value[1] + \'  最低 : \' + params[0].value[2];return res;})");
        }])
        .legendEqual([PYLegend initPYLegendWithBlock:^(PYLegend *legend) {
            legend.dataEqual(@[@"上证指数"]);
        }])
        ...
        .addSeries([PYCartesianSeries initPYCartesianSeriesWithBlock:^(PYCartesianSeries *series) {
            series.nameEqual(@"上证指数")
            .typeEqual(PYSeriesTypeK)
            .addDataArr(@[
...
+ (PYOption *)candlestick2Option {
    return [PYOption initPYOptionWithBlock:^(PYOption *option) {
        ...
        .addSeries([PYCartesianSeries initPYCartesianSeriesWithBlock:^(PYCartesianSeries *series) {
            series.barMaxWidthEqual(@20)
            .nameEqual(@"上证指数")
            .typeEqual(PYSeriesTypeK)
            .itemStyleEqual([PYItemStyle initPYItemStyleWithBlock:^(PYItemStyle *itemStyle) {
                itemStyle.normalEqual([PYItemStyleProp initPYItemStylePropWithBlock:^(PYItemStyleProp *normal) {
                    normal.colorEqual(@"red")
                    .color0Equal(@"lightgreen")
            ...
            .addDataArr(@[
                          @{
                              ...
                              @"itemStyle":[PYItemStyle initPYItemStyleWithBlock:^(PYItemStyle *itemStyle) {
                                                itemStyle.normalEqual([PYItemStyleProp initPYItemStylePropWithBlock:^(PYItemStyleProp *normal) {
                                                    normal.color0Equal(@"blue")
                                                    .lineStyleEqual([PYLineStyle initPYLineStyleWithBlock:^(PYLineStyle *lineStyle) {
            ...
            .markPointEqual([PYMarkPoint initPYMarkPointWithBlock:^(PYMarkPoint *markPoint) {
                markPoint.symbolEqual(PYSymbolStar5)
                .itemStyleEqual([PYItemStyle initPYItemStyleWithBlock:^(PYItemStyle *itemStyle) {
                    itemStyle.normalEqual([PYItemStyleProp initPYItemStylePropWithBlock:^(PYItemStyleProp *normal) {
                ...
                .addData(@{@"name":@"最高", @"value":@2444.8, @"xAxis":@"2013/2/18", @"yAxis":@2466});
            }]);
        }]);
    }];
}
